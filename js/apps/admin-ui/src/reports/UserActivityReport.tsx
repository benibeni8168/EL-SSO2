...
export const UserActivityReport = () => {
  ...
  const buildUrl = (endpoint: string, extraParams?: Record<string, string>) => {
    ...
    return `${adminClient.baseUrl}/admin/realms/${realm}/audit-reports/user-activity/${endpoint}${qs ? "?" + qs : ""}`;
  };

  const generateReport = async () => {
    if (!userId.trim()) {
      setError(t("reportUserId") + " is required");
      return;
    }
    setLoading(true);
    setError(null);
  ...
  const download = async (format: "json" | "csv") => {
    ...
    a.href = url;
    a.download = `user-activity-events.${format}`;
    a.click();
  ...
  return (
    <PageSection>
      <Grid hasGutter>
        <GridItem span={6}>
          <FormGroup label={t("reportUserId")} isRequired>
            <TextInput
              value={userId}
