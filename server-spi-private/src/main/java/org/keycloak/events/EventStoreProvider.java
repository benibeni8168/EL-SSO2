...
public interface EventStoreProvider extends EventListenerProvider {
    ...
    /**
     * Verifies the SHA-256 integrity hash of every stored event for the given realm.
     * For each event the hash is recomputed from its fields and compared to the stored value.
     *
     * <p>Implementations that do not support persistent storage may return a zeroed result.
     *
     * @param realm the realm whose events to verify
     * @return counts of checked, valid, tampered, and legacy (no-hash) events
     */
    default EventIntegrityResult verifyIntegrity(RealmModel realm) {
        return new EventIntegrityResult(0, 0, 0, 0);
    }
