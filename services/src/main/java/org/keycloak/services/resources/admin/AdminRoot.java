...
import org.keycloak.services.resources.admin.backup.BackupAdminResource;
...
public class AdminRoot {
     ...
     *
     * @return BackupAdminResource
     */
    @Path("backups")
    public BackupAdminResource getBackupResource() {
        if (!isAdminApiEnabled()) {
            throw new NotFoundException();
        }
        ...
        if (request.getHttpMethod().equals(HttpMethod.OPTIONS)) {
            throw new NotFoundException(); // Handled by preFlight
        }
        ...
        if (auth != null) {
            logger.debugf("authenticated admin access for backups: %s", auth.getUser().getUsername());
        }

        Cors.builder().allowedOrigins(auth.getToken()).allowedMethods("GET", "PUT", "POST", "DELETE").auth().add();

        // Backups require master realm admin
        RealmModel masterRealm = new RealmManager(session).getKeycloakAdministrationRealm();
        AdminPermissionEvaluator permissionEvaluator = AdminPermissions.evaluator(session, masterRealm, auth);
        return new BackupAdminResource(session, permissionEvaluator);
    }
